...
// ─── VENCIMIENTOS CARD ───────────────────────────────────────────────────────
function VencimientosCard({ vencimientos, onAdd, onDel, onPay }) {
          ...
          <div key={v.id} className={vclass} style={{borderRadius:10,padding:"11px 13px",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0,marginLeft:8}}>
              <div style={{textAlign:"right"}}>
                ...
                {v.recurrente && <div style={{fontSize:9,color:"#c98b90",fontWeight:600}}>↻ recurrente</div>}
              </div>
              <button onClick={()=>onPay(v.id)} title={v.recurrente?"Pagar y avanzar al próximo mes":"Marcar pagado"} style={{background:"#eef8f2",border:"1px solid #a8dcc0",borderRadius:6,color:"#2d8b5a",cursor:"pointer",display:"flex",padding:"4px 6px",fontSize:11,fontWeight:700,alignItems:"center",gap:3}}>✓</button>
      ...
      {view==="dashboard" && (
        <div className="fade-in" style={{padding:"14px 14px 0"}}>
          ...
          <VencimientosCard
            ...
            onAdd={v=>setVencimientos(prev=>[...prev,v])}
            onDel={id=>setVencimientos(prev=>prev.filter(v=>v.id!==id))}
            onPay={id=>setVencimientos(prev=>{
              const v = prev.find(x=>x.id===id);
              if(!v) return prev;
              if(!v.recurrente) return prev.filter(x=>x.id!==id);
              // Avanzar fecha exactamente 1 mes
              const [y,m,d] = v.fecha.split("-").map(Number);
              const next = new Date(y, m, d); // Date(y, m(1-based), d) → JS month es 0-based, así que m sin -1 ya avanza 1 mes
              const pad = n=>String(n).padStart(2,"0");
              const nuevaFecha = `${next.getFullYear()}-${pad(next.getMonth()+1)}-${pad(next.getDate())}`;
              return prev.map(x=>x.id===id ? {...x, fecha: nuevaFecha} : x);
            })}
          />
